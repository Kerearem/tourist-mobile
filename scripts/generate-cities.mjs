/**
 * Generates src/constants/cities.ts from embedded city data.
...
const OUT = path.join(__dirname, "../src/constants/cities.ts");
...
function formatCityEntries(entries) {
  if (!entries.length) return "[]";
  const lines = entries.map((e) => `  { name: "${escapeTsString(e.name)}" }`);
  return "[\n" + lines.join(",\n") + ",\n]";
}

function formatUsStates(states) {
  const lines = states.map((s) => `  { code: "${s.code}", name: "${escapeTsString(s.name)}" }`);
  return "[\n" + lines.join(",\n") + ",\n]";
}

function formatRecordOfCityEntries(record) {
  const keys = Object.keys(record).sort((a, b) => a.localeCompare(b, "en"));
  const parts = keys.map((key) => `  "${key}": ${formatCityEntries(record[key])}`);
  return "{\n" + parts.join(",\n") + ",\n}";
}

function generate() {
  const { US_STATES, US_CITIES_BY_STATE, CITIES_BY_COUNTRY } = PAYLOAD;
  ...
  const body = [
    ...
    "",
    "export const US_STATES: UsStateEntry[] = " + formatUsStates(US_STATES) + ";",
    "",
    "export const CITIES_BY_COUNTRY: CitiesByCountry = " + formatRecordOfCityEntries(CITIES_BY_COUNTRY) + ";",
    "",
    "export const US_CITIES_BY_STATE: UsCitiesByState = " + formatRecordOfCityEntries(US_CITIES_BY_STATE) + ";",
    "",
