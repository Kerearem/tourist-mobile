...
describe("conversation info layout labels", () => {
  it("exposes only implemented quick action labels", () => {
    assert.deepEqual(CONVERSATION_INFO_QUICK_ACTION_LABELS, ["Profil", "Ara"]);
  });

  it("keeps ConversationInfoScreen free of placeholder settings and alerts", () => {
    const source = readFileSync(
      join(process.cwd(), "src/features/messages/screens/ConversationInfoScreen.tsx"),
      "utf8",
    );
    ...
    assert.doesNotMatch(source, /Sessize al/);
    assert.doesNotMatch(source, /Seçenekler/);
    assert.doesNotMatch(source, /Grup sohbeti oluştur/);
    assert.doesNotMatch(source, /Bir şey çalışmıyor/);
    assert.doesNotMatch(source, /yakında/);
  });
});
